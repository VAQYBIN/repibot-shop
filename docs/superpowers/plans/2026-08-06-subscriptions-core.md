...
`uv run check` на ветке `dev` **красный**: схема панели обновлена до 3.2.1, а генератор моделей ищет корневую схему `GetUserByUuidResponseDto`, которой в 3.2.1 нет. Задача 1 чинит это первой, до всего остального.
...
|---|---|---|
| 1 | 1 модели панели, 2 домен подписок, 3 таблицы, 4 настройки | Ни одного общего файла |
| 2 | 5 репозитории, 6 фасады панели | 5 после 3, 6 после 1 и 4 |
| 3 | 7 заглушка панели, 8 сервис тарифов | Обе после 6 |
| 4 | 9 примирение, 10 админское API тарифов | 9 после 5–7, 10 после 8 |
| 5 | 11 сервис подписок | Опирается на 2, 5, 9 |
