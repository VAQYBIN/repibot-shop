...
- **идентификатор Telegram читается из claim `id`, а не из `sub`** — найдено
  ручной проверкой по критерию приёмки №6, подробности во врезке выше;
- тесты читали `.env` разработчика: на машине с настроенным входом через
  Telegram проверки «способ не настроен» падали, а у соседа проходили. Теперь
  `conftest.py` выставляет обе переменные OIDC пустыми явно.
...
Эндпоинта `userinfo` нет: всё, что мы узнаём о человеке, приходит в ID-токене. Имя берётся из `name`, `@username` — из `preferred_username`. Scope `phone` не запрашивается.

> **Идентификатор Telegram лежит в claim `id`, а не в `sub`.** Здесь я ошибся, и ошибка дожила до живого входа: в discovery-документе `id` не объявлен, и я вывел из этого, что идентификатор — `sub`. На самом деле `sub` непрозрачен (в реальном токене — девятнадцатизначное число вроде `1273349464943926156`), а `id` содержит настоящий номер (`728763367`). Цена ошибки: человек, вошедший через браузер, получал второй аккаунт вместо своего и не проходил по списку `ADMIN_TELEGRAM_IDS`. Проверять состав токена нужно по [документации Telegram](https://core.telegram.org/bots/telegram-login), а не по `claims_supported`.
