...
| 8 | 15 `/api/me` и гейт роли | готово |
| 9 | 17 типы API | готово |
| 10 | 18 клиентская аутентификация | готово |
| 11 | 19 экраны веба, 20 экраны MiniApp | готово |
| 12 | 21 сквозные тесты и документация | готово |

Помимо задач плана в этап вошли правки, найденные при сборке:

- настройки отвергают `JWT_SECRET` короче 32 символов;
- проверка портов в `compose.yml` считает рабочий набор — Mailpit живёт под
  профилем `dev`;
- автопоиск Tailwind не доходил от приложений до `packages/ui`, и классы,
  встречающиеся только там, в сборку не попадали: кнопки оставались без заливки;
- ссылка подтверждения нового адреса вела под гейт кабинета и теряла токен —
  страница переехала на публичный `/confirm-email`;
- формы смены почты не существовало, хотя эндпоинт был написан и покрыт тестами;
- `AuthProvider` принимает готовое хранилище токена, поэтому MiniApp пользуется
  общими хуками вместо своих копий;
- код `validation_error` не был переведён, и понятная причина отказа не доходила
  до человека.
