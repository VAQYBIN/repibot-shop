...
Подтверждение почты и сброс пароля отправляются письмом. Способ отправки задаёт
`EMAIL_SENDER`:
...
- `log` (по умолчанию) — письмо не уходит никуда, кроме журнала контейнера `api`
  ...
  искать в логах (`docker compose logs -f worker`) — почтового сервера в
  развёртывании нет.
...
Telegram через запятую. Роль поднимается не сразу при добавлении в список, а
в момент входа: назначенному админу нужно один раз войти через Telegram, чтобы
роль применилась и факт был записан в `audit_log`.
