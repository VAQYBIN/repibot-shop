...
Вход через Telegram появляется в подпроекте 1b: в фундаменте и в 1a его ещё нет,
и отдельных переменных под Client ID и Client Secret пока тоже нет. Настройка при
этом одноразовая и делается при установке — записать оба значения себе удобнее
сразу, чем возвращаться к BotFather потом.
