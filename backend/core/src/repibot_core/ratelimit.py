...
# Обращение в поддержку упирается в человека на той стороне: пауза в десять
# секунд не мешает дописать мысль вторым сообщением, но отсекает скрипт,
# набивающий супергруппу топиками быстрее, чем персонал успевает их читать.
TICKET_MESSAGE_PER_USER = Rule(limit=1, window=timedelta(seconds=10))
