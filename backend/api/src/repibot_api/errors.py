...
_SERVICE_STATUS = {
    ...
    # Публиковать нечего — состояние документа, а не ошибка запроса: тот же
    # запрос пройдёт, как только у пары появится черновик.
    "draft_not_found": 409,
    ...
    # Заглушение — решение персонала о конкретном человеке, а не состояние
    # стенда: повтор того же запроса не поможет, пока его не снимут.
    "support_muted": 403,
    "too_many_tickets": 409,
    # Сотрудника заблокировать нельзя — но запрос при этом составлен верно:
    # отказ говорит о том, кем оказался адресат, а не об ошибке отправителя.
    "staff_immune": 409,
    "ticket_closed": 409,
